...
const CATEGORIES = ['Prenatal', 'Postnatal', 'Baby Care', 'General']
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          <div className="relative">
            <input
              ...
              className={`w-full border rounded-xl px-4 py-3 text-sm text-[#1A1A2E] pr-11 outline-none transition-colors ${
                error
          ...
          <button
            type="submit"
            className="bg-[#2D6A4F] text-white rounded-xl py-3 text-sm font-semibold hover:bg-[#245c43] transition-colors"
          >
            Enter Admin Panel
          </button>
        </form>
...
export default function BlogAdmin() {
  ...
  const handleSubmit = async (published) => {
    ...
    const payload = {
      ...
      read_time: Number(form.readTime) || 5,
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8]">
      ...
      <div className="bg-white border-b border-gray-100 sticky top-16 z-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <PenSquare size={18} className="text-[#2D6A4F]" />
            <span className="font-semibold text-[#1A1A2E] text-sm">Blog Admin Panel</span>
          </div>
          <button
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        {/* ── New post form + preview ── */}
        <div ref={formRef} className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          {/* Left — form */}
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <h2 className="font-['Playfair_Display'] text-xl font-bold text-[#1A1A2E] mb-5">
              Write New Post
            </h2>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                {form.slug && (
                  <p className="text-xs text-[#6B7280] mt-1">
                    URL will be:{' '}
                    <span className="text-[#2D6A4F] font-mono">/blog/{form.slug}</span>
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#1A1A2E] mb-1.5 flex items-center justify-between">
                  ...
                  <span className={`font-normal ${excerptCount > 200 ? 'text-red-400' : 'text-[#6B7280]'}`}>
                    {excerptCount}/200
                  </span>
                </label>
                <textarea
              ...
              <div>
                <label className="block text-xs font-semibold text-[#1A1A2E] mb-1.5">
                  Cover Image URL <span className="text-[#6B7280] font-normal">(optional)</span>
                </label>
                <input
                  id="post-cover-url"
                  name="coverImageUrl"
          ...
          <div className="sticky top-32 self-start bg-white border border-gray-100 rounded-2xl p-5 max-h-[80vh] overflow-y-auto">
            <p className="text-xs text-[#6B7280] mb-3 uppercase tracking-widest font-medium">
              Live Preview
            </p>
            ...
            {form.excerpt && (
              <p className="text-sm text-[#6B7280] mb-4 italic">{form.excerpt}</p>
            )}
            <div className="prose prose-sm max-w-none
              prose-headings:font-['Playfair_Display'] prose-headings:text-[#1A1A2E]
              prose-p:text-[#374151] prose-strong:text-[#1A1A2E]
              prose-li:text-[#374151]
            ">
              <ReactMarkdown>
                {form.content || '_Start writing to see preview…_'}
              </ReactMarkdown>
            </div>
          </div>
        </div>
                ...
                <tbody>
                  {posts.map((post) => (
                    <tr key={post.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-3 pr-4 text-[#6B7280] whitespace-nowrap text-xs">
                      ...
                      <td className="py-3">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {post.published ? (
                              <ToggleRight size={20} className="text-[#52B788]" />
                            ) : (
